...
const obtenerCategorias = async () => {
  const respuesta = await fetch(
    `http://localhost:3001/api/cliente/obtenerCategorias`,
    {
      method: "get",
      headers: {
    ...
    zonas.forEach((zona) => {
      empresas += `<section onclick="mostrarEmpresas('${categoria.nombre}','${zona.zona}','${categoria.nombre}${zona.zona}')" class=" d-flex justify-content-between" style="width: 100%;"><!--Zona inicio-->
            ...
            <div class="d-flex flex-column justify-content-end align-items-center flex-md-row justify-content-sm-end">
                <div onclick= "agregarEmpresa('${categoria.nombre}','${zona.zona}','${categoria._id}')" class="cont-iconos" style="margin-right: 0.3rem;"><i class="fa-solid fa-plus"></i></div>
...
function agregarEmpresa(categoria1, zon, id) {
  console.log("agregar nueva empresa", categoria1, zon, id);
  localStorage.setItem(
    "nuevaEmpresa",
    JSON.stringify({ idCategoria: id, categoria: categoria1, zona: zon })
  );
  window.location.href = "/admin-productos-agregar.html";
}
